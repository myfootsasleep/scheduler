...
describe("Form", () => {
  ...
  xit("calls onSave function when the name is defined", () => {
    const onSave = jest.fn();
    const validName = "Lydia Miller-Jones";
    const validInterviewerId = 1;
    const { queryByText, getByText } = render(<Form onSave = {onSave} interviewers = {interviewers} student = {validName} onCancel ={onSave} interviewer = {validInterviewerId}/>)
